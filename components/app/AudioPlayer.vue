<template>
  ...
  <div v-if="playbackSession" id="streamContainer" class="fixed top-0 left-0 layout-wrapper right-0 z-50 pointer-events-none" :class="{ fullscreen: showFullscreen, 'ios-player': $platform === 'ios', 'web-player': $platform === 'web' }">
    ...
    <div id="playerContent" class="playerContainer w-full z-20 absolute bottom-0 left-0 right-0 p-2 pointer-events-auto transition-all" :style="{ backgroundColor: showFullscreen ? '' : coverRgb }" @click="clickContainer">
      ...
      <div v-if="showFullscreen" class="absolute bottom-4 left-0 right-0 w-full pb-4 pt-2 mx-auto px-6" style="max-width: 414px">
        <div class="flex items-center justify-between pointer-events-auto">
          ...
          <span v-else-if="isPodcast" class="material-symbols text-3xl text-fg-muted cursor-pointer hover:text-accent" title="Podcast Settings" @click.stop="showPodcastSettingsModal = true">tune</span>
    ...
    <!-- Pocket Casts Podcast Settings modal -->
    <modals-podcast-settings-modal v-model="showPodcastSettingsModal" :library-item="libraryItem" @saved="onPodcastSettingsSaved" />
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    onPlaybackSession(playbackSession) {
      ...
      if (this.isPodcast) {
        this.applyPodcastSettings()
      }
    },
    async applyPodcastSettings() {
      const lid = this.libraryItem?.id || this.playbackSession?.libraryItemId
      if (!lid) return
      const settings = await this.$localStore.getPodcastSettings(lid)
      this.currentPodcastSettings = settings
      if (!settings) return

      // Apply custom playback speed if set
      if (settings.customSpeed && !isNaN(settings.customSpeed)) {
        this.selectPlaybackSpeed(settings.customSpeed)
      }

      // Apply silence skipping if enabled
      if (settings.skipSilence !== undefined && AbsAudioPlayer?.setSkipSilence) {
        try {
          await AbsAudioPlayer.setSkipSilence({ enabled: settings.skipSilence })
        } catch (e) {
          console.warn('Failed to set skipSilence', e)
        }
      }

      // Apply Intro Skip (Skip First)
      if (settings.skipFirst && Number(settings.skipFirst) > 0) {
        const introSeconds = Number(settings.skipFirst)
        if (this.currentTime < 5) {
          console.log(`[PocketCasts] Skipping intro by ${introSeconds}s`)
          setTimeout(() => {
            this.seekPlayer(introSeconds)
          }, 350)
        }
      }
    },
    onPodcastSettingsSaved(settings) {
      this.currentPodcastSettings = settings
      if (settings.customSpeed) {
        this.selectPlaybackSpeed(settings.customSpeed)
      }
      if (settings.skipSilence !== undefined && AbsAudioPlayer?.setSkipSilence) {
        AbsAudioPlayer.setSkipSilence({ enabled: settings.skipSilence }).catch(() => {})
      }
    },
    onPlaybackClosed() {
      this.endPlayback()
