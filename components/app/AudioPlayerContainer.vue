<template>
  <div>
    ...
    <modals-sleep-timer-modal v-model="showSleepTimerModal" :current-time="sleepTimeRemaining" :duration="playerDuration" :is-podcast="isPlayerPodcast" :sleep-timer-running="isSleepTimerRunning" :current-end-of-chapter-time="currentEndOfChapterTime" :is-auto="isAutoSleepTimer" @change="selectSleepTimeout" @cancel="cancelSleepTimer" @increase="increaseSleepTimer" @decrease="decreaseSleepTimer" />
...
export default {
  ...
  computed: {
    ...
    isPlayerPodcast() {
      return this.currentPlaybackSession?.mediaType === 'podcast' || !!this.$refs.audioPlayer?.isPodcast
    },
    playerDuration() {
      return this.currentPlaybackSession?.duration || this.$refs.audioPlayer?.totalDuration || 0
    }
  },
  methods: {
    showBookmarks() {
