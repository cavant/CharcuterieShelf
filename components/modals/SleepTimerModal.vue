    ...
    <div class="w-full h-full overflow-hidden absolute top-0 left-0 flex items-center justify-center" @click="show = false">
      <div class="w-full overflow-x-hidden overflow-y-auto bg-primary rounded-lg border border-border" style="max-height: 75%" @click.stop>
        ...
        <ul v-else-if="!sleepTimerRunning" class="h-full w-full" role="listbox" aria-labelledby="listbox-label">
          ...
          <li v-if="currentEndOfChapterTime || (isPodcast && duration)" class="text-fg select-none relative py-4" role="option" @click="clickedChapterOption">
            <div class="flex items-center justify-center">
              <span class="font-normal block truncate text-lg text-center">{{ isPodcast ? 'End of Episode' : $strings.LabelEndOfChapter }}</span>
            </div>
          </li>
...
export default {
  ...
  methods: {
    async clickedChapterOption() {
      ...
      this.show = false
      const remainingSeconds = this.currentEndOfChapterTime || Math.max(1, (this.duration || 0) - (this.currentTime || 0))
      this.$nextTick(() => this.$emit('change', { time: remainingSeconds * 1000, isChapterTime: !!this.currentEndOfChapterTime }))
    },
