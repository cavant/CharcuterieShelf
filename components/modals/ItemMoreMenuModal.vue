<template>
  <div>
    ...
    <modals-item-edit-item-modal v-model="showEditItemModal" :library-item="libraryItem" :initial-tab="editItemInitialTab" @updated="onItemUpdated" />
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    moreMenuItems() {
      ...
      if (this.isConnectedToServer && this.userCanUpdate && !this.episode && this.serverLibraryItemId) {
        items.push({
          text: this.$strings.ButtonEditBook || 'Edit Details',
          value: 'editDetails',
          icon: 'edit'
        })
        ...
        if (!this.isPodcast) {
          items.push({
            text: this.$strings.ButtonEditChapters || 'Edit Chapters',
            value: 'editChapters',
            icon: 'format_list_bulleted'
          })
          items.push({
            text: this.$strings.ButtonMatch || 'Match',
            value: 'matchItem',
            icon: 'auto_fix_high'
          })
        }
        ...
        items.push({
          text: this.$strings.ButtonEditCover || this.$strings.HeaderCover || 'Edit Cover',
          value: 'editCover',
          icon: 'image'
        })
      }
  ...
  methods: {
    moreMenuAction(action) {
      ...
      if (action === 'manageLocal') {
        ...
        this.editItemInitialTab = 'details'
        this.showEditItemModal = true
      } else if (action === 'editChapters') {
        this.editItemInitialTab = 'chapters'
        this.showEditItemModal = true
      } else if (action === 'matchItem') {
        this.editItemInitialTab = 'match'
        this.showEditItemModal = true
