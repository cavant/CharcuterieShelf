<template>
  <div class="w-full max-w-md mx-auto px-2 sm:px-4 lg:px-8 z-10">
    <div v-show="!loggedIn" class="mt-8 bg-primary overflow-hidden shadow rounded-lg px-4 py-6 w-full">
      ...
      <template v-if="!showForm">
        <div v-for="config in serverConnectionConfigs" :key="config.id" class="border-b border-fg/10 py-4">
          <div class="flex items-center my-1 relative space-x-2" @click="connectToServer(config)">
            <div class="grow inline-flex flex-col justify-center overflow-hidden">
              <p class="text-base text-fg truncate">{{ config.name }}</p>
              <div v-if="config.localAddress" class="flex items-center space-x-1 text-xs text-fg-muted mt-0.5">
                <span class="material-symbols text-xs text-success">lan</span>
                <span class="truncate">{{ config.localAddress }}</span>
              </div>
            </div>
            <div class="h-full w-6 flex items-center" @click.stop="openEditConnectionModal(config)">
      ...
      <div v-else class="w-full">
        ...
        <form v-if="!showAuth" @submit.prevent="submit(false)" novalidate class="w-full">
          ...
          <ui-text-input v-model="serverConfig.address" :disabled="processing || !networkConnected || !!serverConfig.id" placeholder="https://abs.example.com or http://55.55.55.55:13378" type="url" class="w-full h-10" />

          <!-- Local Connection Section -->
          <div v-if="!serverConfig.id" class="mt-4 pt-3 border-t border-fg/10">
            <div class="flex items-center justify-between py-1 cursor-pointer" @click="enableLocalConnection = !enableLocalConnection">
              <div>
                <p class="text-sm font-medium text-fg">{{ $strings.LabelEnableLocalConnection || 'Local Network Connection (LAN)' }}</p>
                <p class="text-xs text-fg-muted">{{ $strings.LabelEnableLocalConnectionHelp || 'Use local IP on home Wi-Fi for faster speeds' }}</p>
              </div>
              <ui-toggle-switch v-model="enableLocalConnection" />
            </div>

            <div v-show="enableLocalConnection" class="mt-3 space-y-3">
              <div>
                <p class="text-xs text-fg-muted mb-1">{{ $strings.LabelLocalAddress || 'Local Server Address (IP:Port)' }}</p>
                <ui-text-input v-model="serverConfig.localAddress" :disabled="processing || !networkConnected" placeholder="http://192.168.1.100:13378" type="url" class="w-full h-10" />
              </div>
              <div>
                <p class="text-xs text-fg-muted mb-1">{{ $strings.LabelLocalNetworks || 'Local Wi-Fi Networks (SSIDs)' }}</p>
                <ui-text-input v-model="serverConfig.localNetworks" :disabled="processing || !networkConnected" placeholder="Home-WiFi, Home-WiFi-5G" class="w-full h-10" />
                <p class="text-2xs text-fg-muted mt-1">{{ $strings.LabelLocalNetworksHelp || 'Comma-separated list of Wi-Fi names. Local connection will only be used when connected to these Wi-Fi networks.' }}</p>
                <div class="flex justify-start mt-1.5">
                  <button type="button" class="text-xs text-accent hover:underline flex items-center space-x-1" @click="detectCurrentNetwork">
                    <span class="material-symbols text-xs">wifi_find</span>
    ...
    <modals-edit-server-connection-modal v-model="showEditConnectionModal" :server-connection-config="editingServerConfig" @updated="onServerConnectionUpdated" />
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    showServerList() {
      ...
      this.showAuth = false
      this.error = null
      this.enableLocalConnection = false
      this.serverConfig = {
        address: null,
        remoteAddress: null,
        localAddress: null,
    ...
    async connectToServer(config) {
      ...
      console.log('[ServerConnectForm] connectToServer', config.address, 'local:', config.localAddress)
      this.processing = true
      this.serverConfig = {
        ...config
      }

      // Try resolving local vs remote address
      let targetAddress = config.address
      if (this.$serverAddressResolver) {
        try {
          const resolved = await this.$serverAddressResolver.resolve(config)
          if (resolved?.activeAddress) {
            targetAddress = resolved.activeAddress
            console.log('[ServerConnectForm] Resolved active address:', targetAddress, 'isLocal:', resolved.isLocal)
          }
        } catch (e) {
          console.warn('[ServerConnectForm] Error resolving server address:', e)
        }
      }
      this.serverConfig.address = targetAddress

      this.showForm = true
      var success = await this.pingServerAddress(this.serverConfig.address)
      // Fallback: If ping failed on localAddress, try remote/fallback address
      if (!success && config.localAddress && this.serverConfig.address === config.localAddress) {
        const fallback = config.remoteAddress || config.address
        if (fallback && fallback !== this.serverConfig.address) {
          console.log('[ServerConnectForm] Local address failed ping, trying fallback:', fallback)
          this.serverConfig.address = fallback
          success = await this.pingServerAddress(this.serverConfig.address)
        }
      }
    ...
    async editServerConfig(serverConfig) {
      ...
      if (serverConfig.localAddress) {
        this.enableLocalConnection = true
      }
    ...
    async submit(preventAutoLogin = false) {
      ...
      if (this.enableLocalConnection) {
        this.serverConfig.remoteAddress = this.serverConfig.address
        if (this.serverConfig.localAddress) {
          this.serverConfig.localAddress = this.prependProtocolIfNeeded(this.serverConfig.localAddress)
        }
      } else {
        this.serverConfig.localAddress = null
        this.serverConfig.localNetworks = null
        this.serverConfig.remoteAddress = null
      }
    ...
    async setUserAndConnection({ user, userDefaultLibraryId, serverSettings, ereaderDevices }) {
      ...
      if (this.enableLocalConnection) {
        if (!this.serverConfig.remoteAddress) {
          this.serverConfig.remoteAddress = this.serverConfig.address
        }
      }
    ...
    openEditConnectionModal(config) {
      this.editingServerConfig = { ...config }
      this.showEditConnectionModal = true
    },
    async onServerConnectionUpdated(updatedConfig) {
      console.log('[ServerConnectForm] onServerConnectionUpdated', updatedConfig)
    ...
    async detectCurrentNetwork() {
      try {
        ...
        if (this.$db?.getNetworkInfo) {
          netInfo = await this.$db.getNetworkInfo({ requestPermission: true })
        }
        if (!netInfo && this.$serverAddressResolver) {
          netInfo = await this.$serverAddressResolver.getCurrentNetworkInfo()
        }
        console.log('[ServerConnectForm] Current network info:', netInfo)
        const currentSsid = netInfo?.ssid || netInfo?.wifiSSID
        if (currentSsid && currentSsid !== '<unknown ssid>') {
          const current = this.serverConfig.localNetworks ? this.serverConfig.localNetworks.split(',').map((s) => s.trim()).filter(Boolean) : []
          if (!current.includes(currentSsid)) {
            current.push(currentSsid)
          }
          this.serverConfig.localNetworks = current.join(', ')
        ...
        this.$toast?.error?.('Failed to detect network')
      }
    },
