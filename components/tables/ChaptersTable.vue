<template>
  <div class="w-full my-4">
    <div class="w-full bg-primary px-4 py-2 flex items-center" :class="expanded ? 'rounded-t-md' : 'rounded-md'" @click.stop="clickBar">
      ...
      <div class="flex-grow" />
      <button v-if="userCanUpdate && isConnectedToServer" class="h-8 w-8 rounded-full flex justify-center items-center mr-1 text-gray-300 hover:text-white hover:bg-white/10 active:bg-white/20" :title="$strings.ButtonEditChapters" @click.stop="$emit('editChapters')">
...
export default {
  ...
  computed: {
    ...
    isConnectedToServer() {
      if (!this.libraryItem?.isLocal) return true
