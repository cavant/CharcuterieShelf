...
class MediaManager(private var apiHandler: ApiHandler, var ctx: Context) {
  ...
  private suspend fun checkServerConnection(config:ServerConnectionConfig) : Boolean {
    ...
    if (!successfulPing && !config.remoteAddress.isNullOrEmpty() && config.address != config.remoteAddress) {
      Log.d(tag, "checkServerConnection: Ping failed for ${config.address}, trying fallback remoteAddress ${config.remoteAddress}")
      val remotePing = suspendCoroutine<Boolean> { cont ->
        val remoteConfig = config.copy(address = config.remoteAddress!!)
        apiHandler.pingServer(remoteConfig) {
          Log.d(tag, "checkServerConnection: Checked remote server conn for ${remoteConfig.address} result = $it")
          if (it) {
            config.address = config.remoteAddress!!
