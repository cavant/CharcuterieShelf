...
class AbsAudioPlayer : Plugin() {
  ...
  @PluginMethod
  fun setSkipSilence(call: PluginCall) {
    val enabled = call.getBoolean("enabled", false) ?: false
    Handler(Looper.getMainLooper()).post {
      playerNotificationService.setSkipSilence(enabled)
      val ret = JSObject()
  ...
  @PluginMethod
  fun getSkipSilence(call: PluginCall) {
    val isSkipping = playerNotificationService.getSkipSilence()
    val ret = JSObject()
    ret.put("value", isSkipping)
    call.resolve(ret)
  }
