...
class PlaybackSession(
  ...
  val activeServerAddress: String?
    get() = if (DeviceManager.isConnectedToServer && DeviceManager.serverConnectionConfigId == serverConnectionConfigId && DeviceManager.serverAddress.isNotEmpty()) {
  ...
  fun getContentUri(audioTrack: AudioTrack): Uri {
    ...
    if (checkIsServerVersionGte("2.22.0")) {
      return if (isDirectPlay) {
        Uri.parse("$activeServerAddress/public/session/$id/track/${audioTrack.index}")
      } else {
        // Transcode uses HlsRouter on server
        Uri.parse("$activeServerAddress${audioTrack.contentUrl}")
      }
    }
    return Uri.parse("$activeServerAddress${audioTrack.contentUrl}?token=${DeviceManager.token}")
  }
  ...
  fun getCastMediaMetadata(audioTrack: AudioTrack): com.google.android.gms.cast.MediaMetadata {
    ...
    val coverUri = if (checkIsServerVersionGte("2.17.0")) {
      Uri.parse("$activeServerAddress/api/items/$libraryItemId/cover")
    } else {
      Uri.parse("$activeServerAddress/api/items/$libraryItemId/cover?token=${DeviceManager.token}")
    }
