...
class AbsDatabase : Plugin() {
  ...
  data class ServerConnConfigPayload(val id:String?, val index:Int, val name:String?, val userId:String, val username:String, var version:String, val token:String, val refreshToken:String?, val address:String?, val customHeaders:Map<String,String>?, val remoteAddress:String? = null, val localAddress:String? = null, val localNetworks:String? = null)
  ...
  fun setCurrentServerConnectionConfig(call:PluginCall) {
    ...
    val remoteAddress = serverConfigPayload.remoteAddress ?: call.getString("remoteAddress", null)
    val localAddress = serverConfigPayload.localAddress ?: call.getString("localAddress", null)
    val localNetworks = serverConfigPayload.localNetworks ?: call.getString("localNetworks", null)

    GlobalScope.launch(Dispatchers.IO) {
      if (serverConnectionConfig == null) { // New Server Connection
        val serverAddress = call.getString("address", "").toString()

        // Create new server connection config - use remoteAddress (if provided) or serverAddress for stable sscId
        val baseAddress = if (!remoteAddress.isNullOrEmpty()) remoteAddress else serverAddress
        val sscId = if (!serverConfigPayload.id.isNullOrEmpty()) serverConfigPayload.id else DeviceManager.getBase64Id("$baseAddress@$username")
        ...
        val displayName = if (!remoteAddress.isNullOrEmpty()) "$remoteAddress ($username)" else "$serverAddress ($username)"
        serverConnectionConfig = ServerConnectionConfig(sscId, sscIndex, displayName, serverAddress, serverVersion, userId, username, accessToken, serverConfigPayload.customHeaders, remoteAddress, localAddress, localNetworks)
        ...
        val serverAddress = call.getString("address", null)
        if (!serverAddress.isNullOrEmpty() && serverConnectionConfig?.address != serverAddress) {
          serverConnectionConfig?.address = serverAddress
          shouldSave = true
        }
        if (serverConnectionConfig?.username != username || serverConnectionConfig?.token != accessToken || serverConnectionConfig?.version != serverVersion) {
          ...
          val displayName = if (!remoteAddress.isNullOrEmpty()) "$remoteAddress (${serverConnectionConfig?.username})" else "${serverConnectionConfig?.address} (${serverConnectionConfig?.username})"
          serverConnectionConfig?.name = displayName
          serverConnectionConfig?.version = serverVersion
          serverConnectionConfig?.token = accessToken
          shouldSave = true
        }
        if (remoteAddress != null && serverConnectionConfig?.remoteAddress != remoteAddress) {
          serverConnectionConfig?.remoteAddress = remoteAddress
          shouldSave = true
        }
        if (localAddress != null && serverConnectionConfig?.localAddress != localAddress) {
          serverConnectionConfig?.localAddress = localAddress
          shouldSave = true
        }
        if (localNetworks != null && serverConnectionConfig?.localNetworks != localNetworks) {
          serverConnectionConfig?.localNetworks = localNetworks
          shouldSave = true
        }
  ...
  fun getNetworkInfo(call: PluginCall) {
    if (call.getBoolean("requestPermission", false) == true) {
      if (getPermissionState("location") != PermissionState.GRANTED) {
        requestPermissionForAlias("location", call, "getNetworkInfoPermissionCallback")
        return
      }
    }
    getNetworkInfoInternal(call)
  }

  @PermissionCallback
  fun getNetworkInfoPermissionCallback(call: PluginCall) {
    getNetworkInfoInternal(call)
  }

  private fun cleanSsid(rawSsid: String?): String? {
    if (rawSsid == null) return null
    var clean = rawSsid.trim()
    if (clean.startsWith("\"") && clean.endsWith("\"") && clean.length > 1) {
      clean = clean.substring(1, clean.length - 1)
    }
    return if (clean == "<unknown ssid>" || clean.isEmpty()) null else clean
  }

  private fun getNetworkInfoInternal(call: PluginCall) {
    val result = JSObject()
    try {
      ...
      // Try from modern ConnectivityManager (Android 10+)
      if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q && connManager != null) {
        val network = connManager.activeNetwork
        val capabilities = connManager.getNetworkCapabilities(network)
