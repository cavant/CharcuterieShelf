...
export const actions = {
  ...
  async setupNetworkListener({ state, commit }) {
    ...
    Network.addListener('networkStatusChange', (status) => {
      ...
      commit('setNetworkStatus', status)
      dispatch('checkServerNetworkSwitch')
    })
  ...
  async checkServerNetworkSwitch({ rootState, dispatch }) {
    const config = rootState.user?.serverConnectionConfig
    if (!config || !config.localAddress || !this.$serverAddressResolver) {
      return
    }
    try {
      const result = await this.$serverAddressResolver.resolve(config)
      if (result.activeAddress && result.activeAddress !== config.address) {
        console.log(`[Store] Network switch detected! Switching server address from ${config.address} to ${result.activeAddress} (${result.reason})`)
        const changed = await dispatch('user/updateActiveServerAddress', result.activeAddress, { root: true })
        if (changed) {
          // Reconnect socket to new address
          if (this.$socket) {
            this.$socket.connect(result.activeAddress, config.token)
          }
          // Sync any local playback sessions
          if (this.$db?.syncLocalSessionsWithServer) {
            await this.$db.syncLocalSessionsWithServer(false)
          }
          this.$eventBus?.$emit('server-network-switched', { activeAddress: result.activeAddress, isLocal: result.isLocal })
        }
      }
    } catch (err) {
      console.error('[Store] Error in checkServerNetworkSwitch', err)
    }
  }
}
