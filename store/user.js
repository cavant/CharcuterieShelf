...
export const getters = {
  ...
  getLocalAddress: (state) => {
    return state.serverConnectionConfig?.localAddress || null
  },
  getRemoteAddress: (state) => {
    return state.serverConnectionConfig?.remoteAddress || state.serverConnectionConfig?.address || null
  },
  getLocalNetworks: (state) => {
    return state.serverConnectionConfig?.localNetworks || null
  },
  getIsConnectedViaLocal: (state) => {
    const local = state.serverConnectionConfig?.localAddress
    const current = state.serverConnectionConfig?.address
    return !!(local && current && local === current)
  },
  isMatchingServerAddress: (state) => (serverAddress, serverConnectionConfigId = null) => {
    if (serverConnectionConfigId && state.serverConnectionConfig?.id === serverConnectionConfigId) {
      return true
    }
    if (!serverAddress) return false
    const active = state.serverConnectionConfig?.address
    const local = state.serverConnectionConfig?.localAddress
    const remote = state.serverConnectionConfig?.remoteAddress
    if (active && serverAddress === active) return true
    if (local && serverAddress === local) return true
    if (remote && serverAddress === remote) return true
    return false
  },
...
export const actions = {
  ...
  async updateActiveServerAddress({ state, commit }, newAddress) {
    if (!state.serverConnectionConfig || !newAddress || state.serverConnectionConfig.address === newAddress) {
      return false
    }
    const updatedConfig = {
    ...
    if (savedConfig) {
      ...
      commit('setServerConnectionConfig', updatedConfig)
    }
    return true
  },
