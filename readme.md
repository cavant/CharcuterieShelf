...
[![Download Release APK](https://img.shields.io/badge/Download_APK-v0.14.18--beta-success?style=for-the-badge&logo=android&logoColor=white)](https://github.com/cavant/CharcuterieShelf/releases/latest/download/CharcuterieShelf.apk)
...
- **Auto-Delete Played Episodes**: Automatically deletes downloaded episode audio files from device storage as soon as playback reaches the end, freeing up physical device storage automatically while leaving server files and progress intact. Includes a global toggle in Settings and per-podcast override in Podcast Settings.
- **1-Tap Manual Download Deletion**: Tap the green downloaded badge directly on any episode row in the episode table or latest episodes shelf to instantly delete the local audio file with a confirmation prompt.
