  ...
  <div v-else id="item-page" class="w-full h-full overflow-y-auto overflow-x-hidden relative bg-bg">
    ...
    <div class="relative">
      ...
      <div class="relative z-10 px-3 py-4">
        ...
        <tables-chapters-table v-if="numChapters" :library-item="libraryItem" @playAtTimestamp="playAtTimestamp" @editChapters="openEditChapters" />
    ...
    <!-- modals -->
    <modals-item-more-menu-modal v-model="showMoreMenu" :library-item="libraryItem" :rss-feed="rssFeed" :processing.sync="processing" @updated="loadServerLibraryItem" />

    <modals-item-edit-item-modal v-model="showEditModal" :library-item="libraryItem" :initial-tab="editModalTab" @updated="loadServerLibraryItem" />
...
export default {
  async asyncData({ store, params, redirect, app, query }) {
    ...
    if (libraryItemId.startsWith('local')) {
      ...
      } else if (query.noredirect !== '1' && libraryItem?.libraryItemId && store.getters['user/isMatchingServerAddress'](libraryItem?.serverAddress, libraryItem?.serverConnectionConfigId) && store.state.socketConnected) {
  ...
  computed: {
    ...
    isConnectedToServer() {
      ...
      return this.$store.getters['user/isMatchingServerAddress'](this.libraryItem.serverAddress, this.libraryItem.serverConnectionConfigId)
    },
    canEdit() {
      return this.userCanUpdate && this.isConnectedToServer && !this.isPodcast && !!this.serverLibraryItemId
    },
    isLocal() {
    ...
    serverLibraryItemId() {
      ...
      if (this.isLocalMatchingServerAddress) {
    ...
    /**
     * User is currently connected to a server and this local library item has the same server address or config
     */
    isLocalMatchingServerAddress() {
      if (!this.localLibraryItem || !this.currentServerAddress) return false
      return this.$store.getters['user/isMatchingServerAddress'](this.localLibraryItem.serverAddress, this.localLibraryItem.serverConnectionConfigId)
    },
    /**
    ...
    isLocalMatchingConnectionConfig() {
      ...
      return this.localLibraryItemServerConnectionConfigId === this.currentServerConnectionConfigId || this.isLocalMatchingServerAddress
    },
  ...
  methods: {
    ...
    openEditChapters() {
      this.editModalTab = 'chapters'
      this.showEditModal = true
    },
    itemUpdated(item) {
      if (item && (item.id === this.libraryItemId || item.id === this.serverLibraryItemId)) {
        this.loadServerLibraryItem()
      }
    },
    readBook() {
