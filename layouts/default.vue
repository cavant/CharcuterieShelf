...
export default {
  ...
  methods: {
    ...
    async attemptConnection() {
      ...
      // Resolve active address (local vs remote)
      if (this.$serverAddressResolver && serverConfig.localAddress) {
        try {
          const { activeAddress, isLocal, reason } = await this.$serverAddressResolver.resolve(serverConfig)
          if (activeAddress && activeAddress !== serverConfig.address) {
            AbsLogger.info({ tag: 'default', message: `attemptConnection: Resolved active address to ${activeAddress} (isLocal=${isLocal}, reason=${reason})` })
            serverConfig.address = activeAddress
            serverConfig = (await this.$db.setServerConnectionConfig(serverConfig)) || serverConfig
          }
        } catch (resolveErr) {
          console.error('[default] Error resolving active address', resolveErr)
        }
      }
      ...
      let authRes = await this.$nativeHttp.post(`${serverConfig.address}/api/authorize`, null, nativeHttpOptions).catch((error) => {
        AbsLogger.error({ tag: 'default', message: `attemptConnection: Server auth failed (${serverConfig.name})` })
        return false
      })

      // Fallback: If local authorization failed and remoteAddress is configured, try remote address
      if (!authRes && serverConfig.localAddress && serverConfig.address === serverConfig.localAddress) {
        const fallbackAddress = serverConfig.remoteAddress
        if (fallbackAddress && fallbackAddress !== serverConfig.address) {
          AbsLogger.info({ tag: 'default', message: `attemptConnection: Local auth failed, falling back to remote (${fallbackAddress})` })
          serverConfig.address = fallbackAddress
          authRes = await this.$nativeHttp.post(`${serverConfig.address}/api/authorize`, null, { ...nativeHttpOptions, serverConnectionConfig: serverConfig }).catch((error) => {
            AbsLogger.error({ tag: 'default', message: `attemptConnection: Remote fallback auth also failed` })
            return false
          })
        }
      }
