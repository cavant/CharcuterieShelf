...
class AbsDatabaseWeb extends WebPlugin {
  ...
  async setCurrentServerConnectionConfig(serverConnectionConfig) {
    ...
    if (ssc) {
      ...
      if (serverConnectionConfig.address) ssc.address = serverConnectionConfig.address
      if (serverConnectionConfig.remoteAddress !== undefined) ssc.remoteAddress = serverConnectionConfig.remoteAddress
      if (serverConnectionConfig.localAddress !== undefined) ssc.localAddress = serverConnectionConfig.localAddress
      if (serverConnectionConfig.localNetworks !== undefined) ssc.localNetworks = serverConnectionConfig.localNetworks
      const displayNameAddress = ssc.remoteAddress || ssc.address
      ssc.name = `${displayNameAddress} (${serverConnectionConfig.username || ssc.username})`
      if (serverConnectionConfig.token) ssc.token = serverConnectionConfig.token
      if (serverConnectionConfig.userId) ssc.userId = serverConnectionConfig.userId
      if (serverConnectionConfig.username) ssc.username = serverConnectionConfig.username
      if (serverConnectionConfig.version) ssc.version = serverConnectionConfig.version
      ssc.customHeaders = serverConnectionConfig.customHeaders || ssc.customHeaders || {}
      ...
      const baseAddress = serverConnectionConfig.remoteAddress || serverConnectionConfig.address
      ssc = {
        id: serverConnectionConfig.id || encodeURIComponent(Buffer.from(`${baseAddress}@${serverConnectionConfig.username}`).toString('base64')),
        index: deviceData.serverConnectionConfigs.length,
        name: `${baseAddress} (${serverConnectionConfig.username})`,
        userId: serverConnectionConfig.userId,
        username: serverConnectionConfig.username,
        address: serverConnectionConfig.address,
        remoteAddress: serverConnectionConfig.remoteAddress || serverConnectionConfig.address,
        localAddress: serverConnectionConfig.localAddress || null,
        localNetworks: serverConnectionConfig.localNetworks || null,
