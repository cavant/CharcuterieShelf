...
public class AbsDatabase: CAPPlugin, CAPBridgedPlugin {
    ...
    private var tokenRefreshListener: Any?

    // Used to notify the webview frontend that the token refresh failed
    private var tokenRefreshFailureListener: Any?

    override public func load() {
        super.load()
        // Listen for token refresh events
        tokenRefreshListener = NotificationCenter.default.addObserver(forName: Notification.Name("TokenRefreshed"), object: nil, queue: nil) { [weak self] notification in
            if let accessToken = notification.userInfo?["accessToken"] as? String {
                self?.notifyListeners("onTokenRefresh", data: ["accessToken": accessToken])
            }
        }

        // Listen for token refresh failure events
        tokenRefreshFailureListener = NotificationCenter.default.addObserver(forName: Notification.Name("TokenRefreshFailed"), object: nil, queue: nil) { [weak self] notification in
            let serverConnectionConfigId = notification.userInfo?["serverConnectionConfigId"] as? String
            self?.notifyListeners("onTokenRefreshFailure", data: ["serverConnectionConfigId": serverConnectionConfigId as Any])
        }
        
        ApiClient.notifyListeners = { [weak self] eventName, data in
    ...
    @objc func setCurrentServerConnectionConfig(_ call: CAPPluginCall) {
        ...
        let remoteAddress = call.getString("remoteAddress", "")
        let localAddress = call.getString("localAddress", "")
        let localNetworks = call.getString("localNetworks", "")

        let displayNameAddress = remoteAddress != "" ? remoteAddress : address
        let name = "\(displayNameAddress) (\(username))"
        
        if id == nil {
            id = "\(displayNameAddress)@\(username)".toBase64()
        }
        ...
        config.localNetworks = localNetworks
    ...
    @objc func getNetworkInfo(_ call: CAPPluginCall) {
        ...
        if #available(iOS 14.0, *) {
            NEHotspotNetwork.fetchCurrent { currentNetwork in
                ...
                call.resolve([
                    "isWifi": ipAddress != nil,
                    "ipAddress": (ipAddress != nil ? ipAddress! : NSNull()) as Any,
                    "ssid": (ssid != nil ? ssid! : NSNull()) as Any
                ])
            }
        } else {
            var ssid: String? = nil
