...
- **Auto-Delete Played Episodes**: Automatically deletes downloaded podcast episode audio files from local storage upon playback completion (`PlayerNotificationService.kt:handlePlaybackEnded`), cleaning up empty local library containers in SQLite DB and notifying the webview via `onLocalEpisodeDeleted` without touching server files or listening progress.
- **Configurable Auto-Delete Controls**: Global master toggle in Settings ("Podcasts & Downloads" → "Auto-Delete Played Podcasts", stored in `DeviceSettings` and `$store.state.settings.autoDeletePlayedPodcasts`) and per-podcast override toggle in `PodcastSettingsModal.vue`, enabled by default.
- **Manual Click-to-Delete Downloads**: Interactive `download_done` badge on episode rows (`EpisodeRow.vue`, `LatestEpisodeRow.vue`) allowing 1-tap confirmation and deletion of downloaded media directly from episode lists with instant reactive UI update.
...
- **Active Session Management & Background Sleep Integrity**: Media playback is initiated on-demand by explicit user selection in the UI or Android Auto browse tree. Premature session injection (`restoreLastPlaybackSessionIfNeeded`) is permanently disabled to eliminate background sleep/resume crashes and `ForegroundServiceStartNotAllowedException` on Android 12–15.
